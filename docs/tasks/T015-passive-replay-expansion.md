...
## C25. Part C Implementation Record（Test First + Implementation 实录）

- **提交链**：A `92ccf16`（requestIssues collection 迁移 + Function16 模型 + RED）→ B `e1c9e2c`（Function16 被动语义 + broadcast 集 {0x06,0x10} 接线，GREEN core）→ C `96ed9e2`（QML 多 issue 换行）→ D 本归档。未 amend/squash/push。
- **RED 实证**（A 后）：`modbuslens_passive_tests.exe` → **`Totals: 42 passed, 12 failed`**，12 条全部断言级失败（Function16 在旧链路仍 Unsupported ⇒ Success/matcher/broadcast 期望全破；旧 broadcast 集不认识 0x10）——满足“Function16 old Unsupported → Success 期望 fail、multi-issue、Function16 broadcast 三类 RED”。多 issue 请求侧断言在新 classifyRequest 下先行通过。首次 RED 曾出现 size-guard 缺失引发 SIGSEGV（对空 transactions 解引用），补 `QCOMPARE(transactions.size(), 1)` 后全部转为优雅断言失败——**RED 或 GREEN 都不得以 crash 形态出现**（入档教训）。
- **GREEN**：B 后 passive **54/54**（含 F16-U01~U11、PASSIVE-C01~C17、MULTI-C01/02、BCAST-C01/02），全量 **ctest 24/24**；c16 一处期望值按 REVISED 广播口径修正（completed=6 而非 5：ExpectedNoResponse 属 completed——测试随批准语义走，如实记录）。
- **模型迁移（requestIssue → requestIssues）**：Core 三挂载点（AnalyzedObservedTransaction / ReplayTransactionOutcome / DiagnosisTransaction）均为 `std::vector<TransactionRequestIssue>`；单 issue 即 size==1、无 issue 即 empty；**不存在 singular+plural 双事实源**。`TransactionRequestIssue` 载荷扩为 observed/ minAllowed/ maxAllowedQuantity + observedByteCount/expectedByteCount（uint8，wire 1 字节宽）+ observedLength/expectedLength（**uint16_t**，expected=5+declaredByteCount 可达 260 的类型出台）；新增 code `InvalidRequestByteCount`。`TransactionIssueCode` 新增 `WriteMultipleRegistersEchoMismatch`（复用 expectedRegisterAddress/actualRegisterAddress + expectedQuantity/actualQuantity 四列，token `write_multiple_registers_echo_mismatch`）。
- **Function16（src/core/protocol/Function16.{h,cpp}）**：仅 passive——`readWriteMultipleRegistersFields`（structural，data≥5 可读，保留 odd-byte 事实的 actualValueByteCount）+ `decodeWriteMultipleRegistersResponse`（恰 4 字节）；**无 encoder/wire sender/Serial API/Qt 类型**。
- **Passive 集成**：classifyRequest 按批准的四段顺序产出全量 issues（防 cascade：quantity invalid 不派生 byteCount 期望；payload↔declared 仅需 byteCount 可读）；广播集 {0x06,0x10} 且**不 gate 于语义有效性**（invalid broadcast + NO_RESPONSE = ExpectedNoResponse + requestIssues；任何 response bytes = UnexpectedResponseForBroadcast + requestIssues）；Function16 normal 分支在 FC06 之后、Unsupported fallback 之前；FC08 normal 仍 Unsupported；0x10→0x90 仍走 generic exception（MSB guard 保持）。
- **下游传播**：DiagnosisContext/Replay 复制 collection；Controller `composeIssueText` 按 Core 顺序「；」连接；模型文案（数量=“（124，合法范围 1–123）”、byteCount=“（实际 2 / 期望 4）”、length=“（实际 9 / 期望 7）”、echo=“写多个寄存器响应不匹配（起始地址…；数量…）”）；Agent detail 新 `request_issues` 数组（逐项 code+载荷；旧 singular 键删除）；Prompt 逐条 `request_issue=` 行 + system 语义句族（ExpectedNoResponse 正交表述、byteCount/length/echo mismatch、多 issue 不得只报第一项）；QML secondary 64px/两行 wrap（不裁剪不重叠）。
- **ExpectedNoResponse 语义澄清落地（§26 预列清单同步）**：`TransactionAnalysis.h` 状态注释、`DiagnosisPromptBuilder` system 句、ADR-003 两处语义句全部改为正交表述（“response expectation / observed outcome；request validity 由 requestIssues 表达”）；统计公式**未动**。
...
## C27. Part C Final Acceptance（用户 Manual UI Review = PASS，2026-09-14）

- **Manual UI evidence（刷新后的 deploy + `t015_partc_final.mlog`）**：Dashboard Observed=6/Completed=6/Pending=0，Success=2/Exception=1/ProtocolError=1/ExpectedNoResponse=2，Success Rate=50.0%、Avg=22.0ms；1 条 FC08 normal unsupported 不计入统计且有 notice；Recent Transactions 全项 PASS（Function16 Success、EchoMismatch、Exception 0x03+InvalidRequestQuantity、Success+两 requestIssues、合法广播 ExpectedNoResponse、非法广播 ExpectedNoResponse+requestIssue、multi-issue 换行可读无 clipping/overlap）；Baseline 四点 PASS——`协议错误：1 / 设备异常 0x03：1 / 预期无响应的广播事务：2 / 请求参数不符合协议约束的事务：3`，**affectedCount=3 为事务数而非 issue 总数 4**，**Healthy 未出现**。
- **PROJECT_STATUS 完整性审计**：243 行（合理）；「当前任务」块唯一；LKGC 行恰两处（头部+面板，无冲突）；Phase B/Part C、semantic audit、Function16、multi-request-issue、P0 downstream audit、Manual UI 状态全部在位——无重复 section、无冲突 LKGC。**无需重写，仅做 closure 更新。**
- **T015 defined-parts 结论**：以档案真实标题为准——已定义且全部完成：**Phase A（DONE）、Phase B（DONE）、Part C（DONE）**；grep 证实无 Part D/后续已定义 Part。⇒ **T015 overall = DONE**。
- **Part C 终态**：Learning/Test Design=DONE；Architecture Review=PASS；Implementation=DONE；P0 downstream semantic audit=PASS；Manual UI Review=PASS。Function 0x10 **passive normal semantics = DONE**；Active Serial 仍 FC03 read-only（被动理解≠主动写，源码 grep 锚证零写路径）。
- **最终架构事实（归档版）**：Function 0x10 passive（quantity/byteCount/payload-length 三层 + ordered multi requestIssues）；`TransactionStatus` 与 requestIssues 正交（Success+issues、ExpectedNoResponse+issues 均合法组合）；Statistics 仅描述 outcome 轴；`RequestIssueObserved`=Baseline 确定性 finding；Agent anomaly=status-anomaly OR issues 非空；AI 护栏=Success/高成功率不擦除 request issues；Unsupported 非 ProtocolError；Generic Exception 保留 MSB guard；`.mlog v1` 零语法变化；request-wire corruption 仍 deferred（Gate E）。
- **Git actual bookkeeping（Part C 全链，`git show --name-only` 并集）**：production/build paths=**19**（`CMakeLists.txt` + 18 个 src：analysis 4、diagnosis 3、protocol 2（Function16 新）、replay 2、ui 7）；tests=**7**；docs=**9**（T015 档案/PROJECT_STATUS/BACKLOG/03/02/04/ADR-003/devlog/INTERVIEW_NOTES）。production-affecting=**`da8ce48`**；tests-only verified baseline=**`ae067ab`**；其余 docs-only。
- **verified LKGC 推进 `02ce302` → `ae067ab`**（Architecture PASS + Implementation Review PASS + P0 audit PASS + 自动全链 PASS + deploy PASS + Manual UI PASS；`ae067ab` 为更完整的 verified code/test baseline；docs-only 提交不作 LKGC）。
- 遗留 deferred（非 T015 part）：request-wire corruption per-record 化、Replay v2/timing、UART diagnostics、register-map/business 语义、per-device health、其他功能码——留待未来独立立项。