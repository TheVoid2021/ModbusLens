...
- 已落地（T002–T006）：ctest 注册 9 个测试——`smoke`、`crc`、`frame`、`codec`、`f03`、`simulator`、`simulator_integration`（SIM-I01 全链路闭环）、`fault`（FAULT-T01~T05 含确定性护栏）、`fault_integration`（FAULT-I01/I02：故障层↔解码器联动）；后八者链 `modbuslens_core`（无 Qt 的纯核心库）+ QtTest。
