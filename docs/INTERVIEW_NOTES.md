...
- CRC 实现与查表法原理、按位 vs 查表的时空权衡（T002）
- Modbus RTU 帧切分与 t3.5 定时实现（T002/T005）
- 三模式为什么能共享核心（IFrameSource 抽象落地细节）（T004–T006）
- 事务配对的启发式算法与广播/超时处理（T007）
- 线程模型：采集线程 ↔ 分析线程如何传递数据不丢帧（T004+）
- 虚拟串口对如何做集成测试（T005）
- 只读 Agent 的架构边界如何强制（T010）
- 无锁队列/环形缓冲的使用场景与取舍（T005+）
