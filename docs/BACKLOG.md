...
| M8 | 诊断深化与知识固化 | M8.1 ✅；T014 ✅ Done（verified LKGC `cc8393a`）；T015 🔄（IN PROGRESS，Phase A docs-only AWAITING REVIEW；Gate C 广播语义=用户架构决策）；后续（Replay v2 timing / UART / register-map）⏸ | 🔄 进行中 |
...
| T015 | **Passive Replay Expansion（被动回放诊断扩展）** | M8 | P1 | 🔄 In Progress（**Phase A Learning + Test Design docs-only 完成，AWAITING REVIEW；Phase B 待 Gate A~F 用户裁决；未实现**） | T009、T014（复用 TransactionIssue/下游链） | Phase A 定案：三种坏输入三分（syntax=load fail / captured protocol-invalid=诊断事实 / unsupported≠invalid）；active 与 passive 双契约并存（T007 不改）；Gate A=Core generic passive analyzer（`analyzeObservedTransaction`，FC03 复用、generic exception 只写一处）；Gate B=outcome 级正交 `TransactionRequestIssue`（Scenario 6=Exception 0x03+requestIssue；T014 契约零改动）；Gate C=Broadcast 语义 **USER ARCHITECTURE DECISION**（第七状态 `ExpectedNoResponse` 三案数学，ADR-003 Draft）；Gate D=FC06 先行/0x10 独立 Part C（Function06/Function16 命名定案）；Gate E=Scope A（不吞 request-wire corruption）；Gate F=per-record 化（parser 仍 all-or-nothing）。FC06/0x10 字段布局 repo 未载 ⇒ External Protocol Reference Required（先取证后实现）；PASSIVE-P01~P15 矩阵落库；demo_v1/T014/Active Serial 回归锁；demo_v2 as-is 仍不要求整文件加载。详见 [T015 档案](tasks/T015-passive-replay-expansion.md) + [ADR-003 Draft](../adr/ADR-003-broadcast-outcome-semantics.md) |
...
| 2026-09-13 | **T015 启动：Phase A Learning / Test Design（docs-only，本提交）**——Replay contract 重建 / 坏输入三分 / active-passive 双契约 / Gate A~F / PASSIVE-P01~P15 矩阵 / **ADR-003 Draft（Broadcast=USER ARCHITECTURE DECISION）**；T015 标记 **In Progress，未实现**；Phase B 待用户 Gate 裁决后批准 |
