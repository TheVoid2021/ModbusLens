...
| T004 | **Modbus RTU Codec**（Part A Wire Codec + Part B 0x03 Codec） | M2 | P0 | In Progress — Part A：Test Design ✅（docs-only）/ Implementation ⬜；Part B ⬜ | T003 | **Part A — RTU Wire Codec**：`ModbusRtuFrame` ↔ 完整 wire bytes（CRC 计算 + 低字节在前序列化 + CRC 验证；验收向量 `01 03 00 00 00 01 ↔ 84 0A`；错误模型 `variant<Frame, RtuDecodeError>`，测试矩阵 RTU-A01~A07 已定稿）。**Part B — Function 0x03 Codec**：解释 0x03 的 data 字段（request: startAddress/quantity，大端；normal response: byteCount/寄存器值，大端；exception: 结构化保存 exceptionCode）。两 Part 分别 TDD，**不得一次性实现**。**范围**：fuzz/benchmark 不在 T004；异常码→文字映射归 analysis 层 |
