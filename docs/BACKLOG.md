...
| M8 | 诊断深化与知识固化 | M8.1 ✅（Diagnostic Coverage Audit）；T014 🔄（IN PROGRESS，**Phase B IMPLEMENTED / AWAITING REVIEW**；candidate `cc8393a` 待 Review）；T015 ⏸（Backlog，方向待用户决策） | 🔄 进行中 |
...
| T014 | **Diagnostic Detail Preservation（诊断细节保留）** | M8 | P0 | 🔄 In Progress（**Phase B IMPLEMENTED / AWAITING REVIEW**：`TransactionIssue` 落地 + 三模式继承 + prompt/agent/UI 传播；LKGC candidate `cc8393a` 待用户 Review；Manual UI Smoke WAITING FOR USER） | T007（下游传播涉 T011/T012/T013 既有结构） | Core：`TransactionIssueCode` 七值 + 稀疏载荷 + `TransactionAnalysis.issue`（append-last）+ 六不变量（ProtocolError⇒issue 必有；defensive 用 UnknownProtocolError sentinel 不被下游伪造）。Statistics 逐位不变（STAT-B09）。Replay/Serial 零逻辑改动同漏斗继承。Baseline finding 不变；batch summary = DEFERRED（本轮无消费者，refinement C）。Prompt 行 `issue=<token>`+载荷 additive；AgentTools detail 全量/anomalies 简化 code，hasX 语义；UI 仅 ProtocolError 行 issueText secondary text（adapter 中文，Core 零 QString）。测试：TX a13~a18 新增 + a01~a11 强化、STAT-B09、REPLAY-i05、SERIAL a07/a11/a15/a16、DIAG-A11、AI-B18/B19、AGENT-A11、UI-T01。RED 9→GREEN tx 20/20、ctest 23/23、clean 零警告。详见 [T014 档案](tasks/T014-diagnostic-detail-preservation.md) |
