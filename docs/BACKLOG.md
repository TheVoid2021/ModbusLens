...
| T007 | **Transaction Analysis**（Part A 单事务 + Part B 统计快照） | M4 | P0 | In Progress — **Part A ✅ DONE**（实现+15 测试全绿）/ **Part B ⬜ Not Started** | T004（+T005/T006 提供流量） | **Part A** ✅：`analyzeFunction03Transaction`（六状态 Pending/Success/Exception/CrcError/Timeout/ProtocolError；观察 `variant<Frame, RtuDecodeError, NoResponse>`；跨帧校验地址/功能/数量；elapsed 与 exceptionCode 双不变量经 makeAnalysis 漏斗保证），TX-A01~A12 + I01~I03 全绿。**Part B**：Statistics Snapshot（计数/成功率/延迟摘要）。**范围收缩兑现**：real timer/polling/session manager/database/persistence 未实现；broadcast 口径推迟 Session/Runtime 层 |
