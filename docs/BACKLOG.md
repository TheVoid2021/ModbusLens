...
| T013 | **Final Integration & Demo** | M7 | P1 | In Progress（**Phase E REMEDIATION：串口空态视觉+表格比例列宽；Qt-side 取证 count=0**） | T008（含 T009–T012 可用能力） | 打包/便携发布；演示脚本与素材齐备（demo/ 目录）；文档终稿（README/DEMO_GUIDE 重写）；深色对比度/视觉层级/AI·Agent·Baseline 视觉身份/术语 polish（见 T013 档案 §2/§4）；CRC 查表优化留待单独任务 |
