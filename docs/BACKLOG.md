...
| T002 | **Modbus CRC16** | M2 | P0 | In Progress — Phase A ✅ / Phase B ✅（Test Design）/ Phase C ⬜ | T001.1 | CRC-16/MODBUS **按位法**实现（Poly 0x8005/A001，Init 0xFFFF）；规范向量对拍（"123456789"→0x4B37）；零余数属性测试；空/单字节边界测试。**明确不含**：Frame Codec（T003）、查表法优化（后续优化任务）、benchmark、fuzz testing、simulator、serial port |
