...
| M8 | 诊断深化与知识固化 | M8.1 ✅；T014 ✅ Done（verified LKGC `cc8393a`）；T015 🔄（IN PROGRESS 整体；**Phase A/B ✅ DONE / REVIEW PASS**；verified LKGC `02ce302`；**Part C 0x10 = IN PROGRESS（Learning + Test Design docs-only，AWAITING REVIEW）**）；后续（Replay v2 timing / UART / register-map）⏸ | 🔄 进行中 |
...
| 2026-09-13 | **T015 Phase A Review = PASS（Gate A~F 全批；Gate C 批准第七状态与统计公式）→ Phase B IMPLEMENTED**：docs `c5cfbf7`（口径订正 + 03 §4.5 官方协议回填 + ADR-003 Accepted-candidate）；B `477ed44` RED（passive 8 passed/13 failed）；C `2ba719f` passive core（Function06/PassiveTransactionAnalysis/per-record/统计/Baseline）→ ctest 24/24；D `6944fd5` 下游传播（dashboard/prompt/agent/UI）。clean 152 零警告、qml smoke、deploy+minimal-PATH 全过；Active Serial 写权限零新增；**verified LKGC 推进 `cc8393a` → `02ce302`（用户 Final Review PASS）；T015 整体仍 In Progress（Part C Function 0x10 NOT STARTED）** |
...
| 2026-09-13 | **T015 semantic audit fix（`02ce302`）**：generic exception matcher 增 `(request.fc & 0x80)==0` 守卫（PASSIVE-P16 RED→GREEN；0x88/0x88 → Unsupported）；passive 22/22、ctest 24/24、clean 零警告；**LKGC candidate 刷新为 `02ce302`（`6944fd5` 作废）** |
