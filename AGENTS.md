...
1. **只允许 amend 当前任务最新、尚未 push 的提交**（例：T004 Part A 曾将暂停期间产生的无名提交 amend 为规约信息）。
2. **禁止 rewrite 更早的 Git history**（rebase/filter-branch/删提交等一律不允许——历史档案区原则在 Git 层的延伸）。
3. **当前不得 `git push`**（仓库保持本地；远端与推送策略未来另行决定并记录）。
